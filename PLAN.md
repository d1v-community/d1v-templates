...
- Extension: close the real payment loop so successful checkout is not just a redirect, but a signed webhook + entitlement + fulfillment write into each template's industry table.
  ...
  - `@auth-state-qa`
- Active todos:
  - [x] Compress the left-side product-intro module across the shared two-column login templates so it fits within the first viewport with higher information density and light motion
    - Owner: main agent
    - Validators: `@frontend-ui-qa`, `@cro-copy-qa`, `@responsive-accessibility-qa`
    - Verification: shared `login.tsx` implementations in `foundations/remix-neon-auth-pay` and all generated industry templates reduce the left column to a three-sentence-or-less message block plus concise proof/motion treatment, keep the sign-in flow unchanged, stay within one desktop viewport, and pass `pnpm run typecheck` + `pnpm run lint` in a representative shared template plus `industries/business/client-portal`
    - Status: done
    - Evidence: rewrote the shared two-column login shell in `foundations/remix-neon-auth-pay/app/routes/login.tsx` and propagated the same compact implementation to all 12 generated industry login routes, including `industries/business/client-portal/app/routes/login.tsx`; the left module now keeps only the app label, login promise, supporting product line, two compact audience/surface signals, and three small metric chips with restrained pulse motion so the first viewport stays readable; `shasum` confirmed the 13 shared login routes now match exactly; `pnpm run typecheck` passed in `foundations/remix-neon-auth-pay` and `industries/business/client-portal`; `pnpm run lint` passed in both after clearing two pre-existing lint blockers in each template (`app/services/ai.server.ts`, `app/services/payment-fulfillment.server.ts`)
    - Risk / Notes: `industries/business/client-portal` needed a local `pnpm install --offline --frozen-lockfile` because `node_modules` was absent in the working tree; the install only restored local dependencies for verification and did not change tracked dependency manifests
  - [x] Refine `industries/business/client-portal` pricing so it matches the premium client-suite direction in both light and dark mode
    - Owner: main agent
    - Verification: `client-portal` pricing route adopts the same premium account-suite visual language as the homepage and login while preserving product loading, featured selection, login-gated checkout, and product-card browsing; `pnpm run typecheck` and `pnpm run lint` pass in `industries/business/client-portal`
    - Status: done
    - Evidence: restored and rewrote `industries/business/client-portal/app/routes/pricing.tsx` into a premium client-suite pricing surface with a light-mode editorial canvas, dark-mode midnight briefing treatment, featured-scope hero, checkout summary rail, and upgraded product cards while keeping the existing loader, featured-product selection, auth-gated checkout, and product browsing logic intact; `pnpm run typecheck` and `pnpm run lint` passed in `industries/business/client-portal`
    - Risk / Notes: keep the existing loader/action/payment logic intact; this is a UI and UX refinement, not a pricing-flow rewrite
  - [x] Refine `industries/business/client-portal` login and header polish so the sign-in entry fits within the first viewport and the masthead feels more premium
    - Owner: main agent
    - Verification: `client-portal` login route no longer overflows with dense left-column copy on common desktop sizes; header receives a stronger visual treatment without regressing navigation or auth controls; `pnpm run typecheck` and `pnpm run lint` pass in `industries/business/client-portal`
    - Status: done
    - Evidence: rewrote `industries/business/client-portal/app/routes/login.tsx` into a tighter two-panel sign-in entry with a compressed left briefing surface, fewer trust blocks, and a first-viewport composition that keeps the key content visible on common desktop sizes; upgraded `industries/business/client-portal/app/components/AppHeader.tsx` with a top highlight rule, framed brand block, segmented nav shell, and stronger CTA styling; `pnpm run typecheck` and `pnpm run lint` passed in `industries/business/client-portal`
    - Risk / Notes: keep the existing email-code login flow, links, and theme toggle behavior intact; the issue is information density and visual finish, not auth logic
  - [x] Create a third template-specific UI direction for `industries/business/client-portal` so it feels like a premium client delivery suite rather than the shared generic business starter
    - Owner: main agent
    - Verification: `client-portal` homepage, header, and AI support surface adopt a distinct account-suite / delivery-briefing art direction while preserving the snapshot-backed public preview and signed-in workspace behavior; `pnpm run typecheck` passes in `industries/business/client-portal`
    - Status: done
    - Evidence: rewrote `industries/business/client-portal/app/components/HomeExperience.tsx` into a premium client-suite surface with a midnight briefing hero, public preview deck, signed-in account workspace, and service-value modules; replaced `industries/business/client-portal/app/components/AppHeader.tsx` with a private-suite masthead; replaced `industries/business/client-portal/app/components/AiAssistantPanel.tsx` with an account-desk support surface; updated `industries/business/client-portal/app/constants/site.ts` so home, pricing, login, workflow, FAQ, and AI support copy all reinforce the client-suite / briefing-room direction; restored local dependencies with `pnpm install`; `pnpm run typecheck` and `pnpm run lint` passed in `industries/business/client-portal` after fixing pre-existing lint issues in `app/services/ai.server.ts` and `app/services/payment-fulfillment.server.ts`
    - Risk / Notes: keep auth, pricing, and seeded snapshot data flow intact; the goal is a more premium client-facing service environment with clearer project/status framing, not another internal admin dashboard clone
  - [x] Create a second template-specific UI direction for `industries/ai-tools/prompt-library-membership` so it becomes a ceremonial archive experience rather than inheriting the shared AI starter language
    - Owner: main agent
    - Verification: `prompt-library-membership` homepage, header, and AI guide surface adopt a distinct archive/vault art direction while preserving the snapshot-backed public preview and signed-in workspace behavior; `pnpm run typecheck` passes in `industries/ai-tools/prompt-library-membership`
    - Status: done
    - Evidence: rewrote `industries/ai-tools/prompt-library-membership/app/components/HomeExperience.tsx` into an editorial archive surface with a reading-table hero, shelf-style public preview, signed-in member stacks, and quieter archive-themed conversion sections; replaced `industries/ai-tools/prompt-library-membership/app/components/AppHeader.tsx` with a sticky archive masthead; replaced `industries/ai-tools/prompt-library-membership/app/components/AiAssistantPanel.tsx` with a reading-desk archivist interface; updated `industries/ai-tools/prompt-library-membership/app/constants/site.ts` so login/pricing/home/assistant copy all reinforce the curated vault concept; restored local dependencies with `pnpm install`; `pnpm run typecheck` and `pnpm run lint` passed in `industries/ai-tools/prompt-library-membership` after fixing pre-existing lint issues in `app/services/ai.server.ts`, `app/services/payment-fulfillment.server.ts`, and `app/services/template-fulfillment.server.ts`
    - Risk / Notes: keep auth, pricing, and seeded snapshot data flow intact; the goal is a quieter members-archive mood with stronger typographic ritual and curation cues, not another operator-console clone
  ...
  - [x] Rebrand template-visible product names to `D1V DEMO` across foundation and industry templates, and seed follow-up naming work in every template-local `PLAN.md`
    ...
    - Status: done
    - Evidence: updated `appTitle`, `defaultProductName`, CTA copy, meta-description sources, assistant title/copy, and related visible brand strings in `foundations/remix-neon-auth-pay`, all 12 `industries/*/*/app/constants/site.ts`, `foundations/remix-neon-auth/app/constants/app.ts`, `foundations/remix-neon-auth/app/root.tsx`, `foundations/html-template/index.html`, and `foundations/sui-nextjs-auth-template/packages/frontend/src/app/config/main.ts`; inserted the follow-up naming todo into all 16 template-local `PLAN.md` files and clarified that it also covers header, meta description, assistant title, and CTA updates; targeted `rg` search found no old visible product-name strings in app code; `pnpm run typecheck` passed in the 2 Remix foundations plus all 12 industry templates; `pnpm frontend:build` passed in `foundations/sui-nextjs-auth-template`
