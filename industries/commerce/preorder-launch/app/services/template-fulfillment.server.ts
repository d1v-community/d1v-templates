...
export async function fulfillTemplateEntitlement(
  ...
  const recordId = buildBusinessRecordId("preorder_paid", context);
  await db.execute(
    sql`insert into preorders (id, offer_id, customer_email, status, reservation_label) values (${recordId}, ${"offer_alpha_drop"}, ${emailFor(context)}, ${"confirmed"}, ${`Reserved via ${productLabelFor(context)}`}) on conflict (id) do update set
      offer_id = excluded.offer_id,
      customer_email = excluded.customer_email,
      status = excluded.status,
      reservation_label = excluded.reservation_label,
      updated_at = now()`,
  );
